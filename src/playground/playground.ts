...
// import {PlaygroundFacadeNew} from '../app/facades/playground.facade.new';
import {Coolearning} from '../app/coolearning';
import {PlaygroundFacade} from '../app/new/facades/playground.facade';

const coolearning = new Coolearning();
...
function drawNode(cx: number, cy: number, nodeId: string, isInput: boolean,
  ...
  let div = d3.select('#network').insert('div', ':first-child')
    ...
    .on('mousedown', () => {
      mouseTimer = setTimeout(() => {
        ...
        // coolearning.views.network.toggleNeuron(parseInt(nodeId));
        clearTimeout(mouseTimer);
        mouseTimer = null;
      }, 600);
    })
    .on('mouseup', () => {
      ...
      if (Number.isNaN(parseInt(nodeId))) {
        ...
        if (!div.classed('selected')) {
          // coolearning.views.network.toggleNodeSelection(parseInt(nodeId), true);
        }
        else {
          // coolearning.views.network.toggleNodeSelection(parseInt(nodeId), false);
        }
      }
  ...
  if (isInput) {
    div.on('click', function() {
      // coolearning.views.network.toggleInput(nodeId);
    });
  }
  if (isInput) {
...
function addPlusMinusControl(x: number, layerIdx: number) {
  ...
  // playgroundUi.init();
  // coolearning.views.initFromPlayground();
}
...
function reset(onStartup = false) {
  ...
  drawNetwork(network);
  updateUI(true);
  // if (coolearning.store.devices.pickedSelector) {
  //   // selectorDevice.drawNeurons();
  //   coolearning.devices.selector.drawNeurons();
  // }
  // if (coolearning.store.devices.pickedController) {
  //   // controllerDevice.updateMode();
  //   coolearning.devices.controller.updateMode();
  // }
}
